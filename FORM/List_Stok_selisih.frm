...
ElseIf lblkode = "BELI" Then
sqlCS1 = "select kdbarang,sum(U_beli + U_Rpinjam + U_Rsewa + M_unit - U_free - U_pinjam - U_sewa  - K_unit - repair) as Unit,0 as UKeluar" & vbCrLf & _
         "from RKP_stok where kdgudang='" & Beli_D.lblkdgudang & "' and tgl <= '" & Format(Beli_D.txttglBPB, "yyyy/MM/dd") & "' and  kdbarang in (select kdbarang from beli_d where kdbeli='" & Beli_D.txtkdPO & "_" & Beli_D.lblnoBPB & "') group by kdbarang" & vbCrLf & _
         "Union All" & vbCrLf & _
         "select kdbarang,0 as unit,unit as UKeluar from beli_d where kdbeli='" & Beli_D.txtkdPO & "_" & Beli_D.lblnoBPB & "'"
