...
Begin VB.Form Customer 
   ...
   Begin Threed.SSCommand cmdT 
      ...
      Left            =   17775
      TabIndex        =   5
      ToolTipText     =   "Monitoring Form SPP"
      Top             =   6030
      Width           =   780
      _ExtentX        =   1376
      _ExtentY        =   1455
...
Customer_TU.cmdMR.Enabled = False
...
Customer_TU.cmdMR.Enabled = True
...
If Opt1.Value = True Then
    If ChkR.Value = 0 Then
        If TXTCARI = "" Then
        sql = "select a.*,b.nmareaC,c.nmteknisi,N_aktif = case when a.non_aktif=1 then 'X' else '' end,isnull(d.custnm,'') as nmcustomer_IAP from Customer a left join area_cheker b on a.kdareaC=b.kdareaC left join teknisi c on a.kdteknisi=c.kdteknisi left join " & rsDbase!nmDbase & "..Vcustomer_IAP d on  convert(varchar,a.kdSP) + '/' + a.kdcustomer_IAP=d.kdcust_iap order by a.kdcustomer desc"
        Else
        sql = "select a.*,b.nmareaC,c.nmteknisi,N_aktif = case when a.non_aktif=1 then 'X' else '' end,isnull(d.Custnm,'') as nmcustomer_IAP from Customer a left join area_cheker b on a.kdareaC=b.kdareaC left join teknisi c on a.kdteknisi=c.kdteknisi left join " & rsDbase!nmDbase & "..Vcustomer_IAP d on  convert(varchar,a.kdSP) + '/' + a.kdcustomer_IAP=d.kdcust_iap where " & kategori & " like '%" & TXTCARI & "%' order by a.kdcustomer desc"
        End If
    Else
        If TXTCARI = "" Then
        sql = "select TOP " & CLng(txtR) & " a.*,b.nmareaC,c.nmteknisi,N_aktif = case when a.non_aktif=1 then 'X' else '' end,isnull(d.Custnm,'') as nmcustomer_IAP from Customer a left join area_cheker b on a.kdareaC=b.kdareaC left join teknisi c on a.kdteknisi=c.kdteknisi left join " & rsDbase!nmDbase & "..Vcustomer_IAP d on  convert(varchar,a.kdSP) + '/' + a.kdcustomer_IAP=d.kdcust_iap order by a.kdcustomer desc"
        Else
        sql = "select TOP " & CLng(txtR) & " a.*,b.nmareaC,c.nmteknisi,N_aktif = case when a.non_aktif=1 then 'X' else '' end,isnull(d.Custnm,'') as nmcustomer_IAP from Customer a left join area_cheker b on a.kdareaC=b.kdareaC left join teknisi c on a.kdteknisi=c.kdteknisi left join " & rsDbase!nmDbase & "..Vcustomer_IAP d on  convert(varchar,a.kdSP) + '/' + a.kdcustomer_IAP=d.kdcust_iap where " & kategori & " like '%" & TXTCARI & "%' order by a.kdcustomer desc"
        End If
    End If
Else
    If ChkR.Value = 0 Then
        If TXTCARI = "" Then
        sql1 = "select a.*,b.nmareaC,c.nmteknisi,N_aktif = case when a.non_aktif=1 then 'X' else '' end,isnull(d.Custnm,'') as nmcustomer_IAP from Customer a left join area_cheker b on a.kdareaC=b.kdareaC left join teknisi c on a.kdteknisi=c.kdteknisi left join " & rsDbase!nmDbase & "..Vcustomer_IAP d on  convert(varchar,a.kdSP) + '/' + a.kdcustomer_IAP=d.kdcust_iap "
        Else
        sql1 = "select a.*,b.nmareaC,c.nmteknisi,N_aktif = case when a.non_aktif=1 then 'X' else '' end,isnull(d.Custnm,'') as nmcustomer_IAP from Customer a left join area_cheker b on a.kdareaC=b.kdareaC left join teknisi c on a.kdteknisi=c.kdteknisi left join " & rsDbase!nmDbase & "..Vcustomer_IAP d on  convert(varchar,a.kdSP) + '/' + a.kdcustomer_IAP=d.kdcust_iap where " & kategori & " like '%" & TXTCARI & "%'"
        End If
...
ElseIf Index = 6 Then
List_Customer_SPP.Show vbModal
End If
End Sub
