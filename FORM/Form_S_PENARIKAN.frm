...
sql = "select kdcust_iap,bln as bulan,sum(case kat when 'GLN' then qty else 0 end) as qty_gln " & vbCrLf & _
      ",sum(case kat when 'CUP' then qty else 0 end) as qty_Cup,sum(case kat when 'BTL' then qty else 0 end) as qty_btl from " & cmbdbase & "..V_omset_TSP where kdcust_iap='" & Customer_TU.lblkdcustomer_IAP & "' group by kdcust_iap,bln order by bln"
...
With AR_S_PENARIKAN
.fldBLN.DataField = "bulan"
.fldgln.DataField = "qty_gln"
.fldcup.DataField = "qty_cup"
.fldbtl.DataField = "qty_btl"

.lblket_omset = "Tahun : " & Right(cmbdbase.Text, 4)
...
sql1 = "Select * from Dbase_RPT order by urutan"
Set rs1 = con.Execute(sql1)

rs1.MoveFirst

Do While Not rs1.EOF
cmbdbase.AddItem rs1!nmDbase
rs1.MoveNext
Loop
