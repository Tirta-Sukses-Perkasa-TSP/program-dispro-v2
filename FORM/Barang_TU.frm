...
Begin VB.Form Barang_TU 
   ...
   Begin VB.Label Label15 
      ...
      Width           =   1140
   End
   Begin VB.Label lblstatus_kepemilikan 
      Appearance      =   0  'Flat
      BackColor       =   &H80000005&
   ...
   Begin VB.Label Label12 
      ...
      Width           =   960
   End
   Begin VB.Label lblkdbarang_mars 
      Appearance      =   0  'Flat
      BackColor       =   &H80000005&
      BorderStyle     =   1  'Fixed Single
      BeginProperty Font 
...
'On Error GoTo hell
    ...
    Else
        ...
        If cmbkategori.Text = "04" Or cmbkategori.Text = "05" Or cmbkategori.Text = "06" Or cmbkategori.Text = "07" Then
        lbljnsbrg_MARS = "DISPENSER"
        ElseIf cmbkategori.Text = "08" Or cmbkategori.Text = "09" Then
        lbljnsbrg_MARS = "SHOWCASE"
        ElseIf cmbkategori.Text = "10" Then
        lbljnsbrg_MARS = "RAK GALON"
        Else
        lbljnsbrg_MARS = ""
        End If
        
        If CInt(cmbkategori.ListIndex) > 2 And TXTKD1 <> "" Then
        lblstatus_kepemilikan = "SEWA"
        lblkdbarang_mars = TXTKD1
        ElseIf CInt(cmbkategori.ListIndex) > 2 And txtkdSAP <> "" Then
        lblstatus_kepemilikan = "MSA"
        lblkdbarang_mars = txtkdSAP
        ElseIf CInt(cmbkategori.ListIndex) > 2 And txtkdSAP = "" And TXTKD1 = "" Then
        lblstatus_kepemilikan = "MSNA"
        lblkdbarang_mars = lblkdbarang
        Else
        lblstatus_kepemilikan = ""
        lblkdbarang_mars = ""
        End If
        ...
        Else
             If LBLKODE = 1 Then
                 sql = "insert into barang values ('" & Trim(UCase(lblkdbarang)) & "','" & UCase(TXTnmbarang) & "','" & UCase(txtsatuan) & "','" & lblkdkategori & "','" & UCase(txtketerangan) & "'," & ChkBFS.Value & "," & CCur(txtBFS) & ",'" & UCase(TXTKD1) & "'," & ChkNA.Value & ",'" & UCase(txtmerk) & "','" & UCase(txtkdSAP) & "','1900/01/01','" & lblkdbarang_mars & "','" & lblstatus_kepemilikan & "','" & lbljnsbrg_MARS & "')"
                 con.Execute (sql)
...
'                    Exit Sub
'                 Else
                    sql = "update barang set nmbarang='" & UCase(TXTnmbarang) & "',satuan='" & UCase(txtsatuan) & "',keterangan='" & UCase(txtketerangan) & "',kdkategori='" & lblkdkategori & "',BFS=" & ChkBFS.Value & ",Unit_BFS=" & CCur(txtBFS) & ",kd1='" & UCase(TXTKD1) & "',Non_aktif = " & ChkNA.Value & ",tglnon_aktif='" & Format(txttglnon_aktif, "yyyy/MM/dd") & "',merk='" & UCase(txtmerk) & "',kdSAP='" & UCase(txtkdSAP) & "',kdbarang_mars='" & lblkdbarang_mars & "',status_kepemilikan='" & lblstatus_kepemilikan & "' , jnsbrg_mars='" & lbljnsbrg_MARS & "' where kdbarang='" & lblkdbarang & "'"
                    con.Execute (sql)
