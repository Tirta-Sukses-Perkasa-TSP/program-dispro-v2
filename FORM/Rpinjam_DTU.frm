    ...
    Else
         ...
         If lblkode = 1 Then
           ...
           If CCur(lblmaxunit) < CCur(txtunit) Then
             ...
             Exit Sub
           Else
             sql = "insert into Rpinjam_d values ('" & UCase(lblkdbarang) & "_" & UCase(Rpinjam_D.lblKDRPinjam) & "','" & UCase(Rpinjam_D.lblKDRPinjam) & "','" & UCase(lblkdbarang) & "'," & CCur(txtunit) & "," & CCur(txtharga) & "," & CCur(lblrupiah) & ",'" & UCase(txtketerangan) & "','" & lblkdPinjam & "',getdate())"
             con.Execute (sql)
