    ...
    Else
         ...
         If lblkode = 1 Then
           ...
           If CCur(lblmaxunit) < CCur(txtunit) Then
             ...
             Exit Sub
           Else
             sql = "insert into Rsewa_d values ('" & UCase(lblkdbarang) & "_" & UCase(RSewa_d.lblKDRsewa) & "','" & UCase(RSewa_d.lblKDRsewa) & "','" & UCase(lblkdbarang) & "'," & CCur(txtunit) & "," & CCur(txtharga) & "," & CCur(lblrupiah) & ",'" & UCase(txtketerangan) & "','" & lblkdsewa & "',getdate())"
             con.Execute (sql)
